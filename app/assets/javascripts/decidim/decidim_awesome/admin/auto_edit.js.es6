...
$(() => {
  $("body").on("click", "a.awesome-auto-edit", (e) => {
    ...
    const $privateMultiple = $(
      `[name="config[private_${attribute}][${key}][]"]`
    );
    ...
    const rebuildHmtl = (result) => {
      ...
      $privateHidden.attr(
        "name",
        `config[private_${attribute}][${result.key}]`
      );
      $multiple.attr("name", `config[${attribute}][${result.key}][]`);
      $privateMultiple.attr(
        "name",
        `config[private_${attribute}][${result.key}][]`
      );
      ...
      $delete.attr(
        "href",
        $delete.attr("href").replace(`key=${key}`, `key=${result.key}`)
      );
    ...
    $target.html(
      `<input class="awesome-auto-edit" data-scope="${scope}" type="text" value="${key}">`
    );
    ...
    $input.on("keypress", (e) => {
      ...
      if (e.code == "Enter") {
        e.preventDefault();
        $.post(
          DecidimAwesome.rename_scope_label_path,
          { key: key, scope: scope, attribute: attribute, text: $input.val() },
