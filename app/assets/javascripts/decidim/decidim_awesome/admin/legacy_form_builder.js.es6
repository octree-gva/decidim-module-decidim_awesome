...
$(() => {
  $(".awesome-edit-config .proposal_custom_fields_editor").each((_idx, el) => {
    ...
    formBuilderList.push({
      ...
      config: {
        ...
        formData: $(
          `input[name="config[proposal_custom_fields][${key}]"]`
        ).val(),
  ...
  $("form.awesome-edit-config").on("submit", () => {
    ...
    formBuilderList.forEach((builder) => {
      $(`input[name="config[proposal_custom_fields][${builder.key}]"]`).val(
        builder.instance.actions.getData("json")
      );
    });
  });
});
