...
((exports) => {
  ...
  if (
    exports.DecidimAwesome.allow_images_in_full_editor ||
    exports.DecidimAwesome.allow_images_in_small_editor ||
    exports.DecidimAwesome.use_markdown_editor
  ) {
    const quillFormats = [
      "bold",
      "italic",
    ...
    const createQuillEditor = (container) => {
      ...
      if (addImage) {
        ...
        $(container).after(
          `<p class="help-text" style="margin-top:-1.5rem;">${t}</p>`
        );
      }
    };
    ...
    const createMarkdownEditor = (container) => {
      ...
      if (window.DecidimAwesome.allow_images_in_markdown_editor) {
        $(inscrybmde.gui.statusbar).prepend(
          `<span class="help-text" style="float:left;margin:0;text-align:left;">${t}</span>`
        );
