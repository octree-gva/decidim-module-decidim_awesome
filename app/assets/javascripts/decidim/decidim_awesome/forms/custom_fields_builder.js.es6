...
class CustomFieldsBuilder {
  ...
  async dataToXML(data) {
    ...
    for (key in data) {
      ...
      if (data[key].type === "attachFile") {
        ...
        const needToUpload =
          $(`#${data[key].name}-input`).attr("type") == "file" &&
      ...
      if (data[key].userData && data[key].userData.length) {
        ...
        for (val in data[key].userData) {
          ...
          text = null;
          if (data[key].values) {
            l = data[key].values.find(
              (v) => v["value"] == data[key].userData[val]
            );
            if (l) {
              text = label;
              label = l.label;
  ...
  fixBuggyFields() {
     ...
     */
    this.$container
      .find(".formbuilder-radio input.other-val")
      .on("input", (input) => {
