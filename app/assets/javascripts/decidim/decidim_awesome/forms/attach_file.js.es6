  ...
  class controlAttachFile extends controlClass {
    ...
    build() {
      ...
      const fileControl = this.markup("input", null, {
        ...attrs,
        id: `${this.config.name}-input`,
        type: "file",
      });
      if (!currentValue) this.input = fileControl;
      else {
        ...
        `,
          {
            class: "formBuilder__attachField-button",
            title: "remove file",
            alt: "remove",
          }
        );
        ...
        const link = this.markup("a", currentValue, {
          href: currentValue,
          target: "_blank",
          class: "formBuilder__attachField-preview",
        });
        const input = (this.input = this.markup(
          "div",
          [link, hiddenInput, removeButton],
          { class: "formBuilder__attachField" }
        ));
