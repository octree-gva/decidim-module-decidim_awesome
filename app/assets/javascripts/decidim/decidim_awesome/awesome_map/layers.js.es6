...
((exports) => {
  ...
  const addProposalsControls = (map, component) => {
    ...
    layers.proposals = {
      label: `<span class="awesome_map-component" id="awesome_map-component_${
        component.id
      }" title="0">${
        component.name || window.DecidimAwesome.texts.proposals
      }</span>`,
  ...
  const addMeetingsControls = (map, component) => {
    ...
    layers.meetings = {
      label: `<span class="awesome_map-component" id="awesome_map-component_${
        ...
        component.name || window.DecidimAwesome.texts.meetings
      }</span>`,
      group: L.featureGroup.subGroup(cluster),
    };
  ...
  const addSearchControls = () => {
    $(control.getContainer()).contents("form")
      .after(`<div id="awesome_map-categories-control" class="active"><b class="awesome_map-title-control">${window.DecidimAwesome.texts.categories}</b><div class="categories-container"></div></div>
      ...
      categories.forEach((category) => {
        ...
        setTimeout(() => {
          $("#awesome_map-categories-control .categories-container").append(
            `<label data-layer="${category.id}" class="awesome_map-category-${
              category.id
            }${
              category.parent ? " subcategory" : ""
            }"><input type="checkbox" class="awesome_map-categories-selector" checked><span>${label}</span></label>`
          );
        });
      });
    }
      ...
      hashtags.forEach((hashtag) => {
        ...
        if (!layers[hashtag.tag]) {
          ...
          $("#awesome_map-hashtags-control .hashtags-container").append(
            `<label data-layer="${hashtag.tag}" class="awesome_map-hashtag-${hashtag.tag}"><input type="checkbox" class="awesome_map-hashtags-selector" checked><span>${hashtag.name}</span></label>`
          );
          // Call a trigger, might be in service for customizations
          exports.AwesomeMap.hashtagAdded(
            hashtag,
        ...
        $label.attr(
          "title",
          parseInt($label.attr("title") || 0) +
            1 +
            " " +
            window.DecidimAwesome.texts.items
        );
      });
    }
  };
